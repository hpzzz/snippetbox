...
func TestShowSnippet(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid ID", "/snippet/1", http.StatusOK, []byte("test title")},
...
func TestSignupUser(t *testing.T) {
	//Create the application struct containing our mocked dependencies and set up the test server for running and end-to-end test
	app := newTestApplication(t)
	ts := newTestServer(t, app.routes())
	defer ts.Close()

	//Make a GET /user/signup request and then request the CSRF token from the response body
